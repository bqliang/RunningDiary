...
//            return -1;
    ...
    public static float calculateLineDistance(LatLng start, LatLng end) {
        if (start != null && end != null) {
            try {
                double degreeToRadian = Math.PI / 180;
                double startLng = start.longitude * degreeToRadian;
                double startLat = start.latitude * degreeToRadian;
                double endLng = end.longitude * degreeToRadian;
                double endLat = end.latitude * degreeToRadian;

                double startLngSin = Math.sin(startLng);
                double startLatSin = Math.sin(startLat);
                double startLngCos = Math.cos(startLng);
                double startLatCos = Math.cos(startLat);
                double endLngSin = Math.sin(endLng);
                double endLatSin = Math.sin(endLat);
                double endLngCos = Math.cos(endLng);
                double endLatCos = Math.cos(endLat);
                double[] startPointVector = new double[3];
                double[] endPointVector = new double[3];
                startPointVector[0] = startLatCos * startLngCos;
                startPointVector[1] = startLatCos * startLngSin;
                startPointVector[2] = startLatSin;
                endPointVector[0] = endLatCos * endLngCos;
                endPointVector[1] = endLatCos * endLngSin;
                endPointVector[2] = endLatSin;
                return (float) (Math.asin(Math.sqrt((startPointVector[0] - endPointVector[0]) * (startPointVector[0] - endPointVector[0]) + (startPointVector[1] - endPointVector[1]) * (startPointVector[1] - endPointVector[1]) + (startPointVector[2] - endPointVector[2]) * (startPointVector[2] - endPointVector[2])) / 2.0) * 1.27420015798544E7);
